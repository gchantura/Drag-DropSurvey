...
export type Alignment = "left" | "center" | "right" | "top" | "middle" | "bottom" | "middleLeft" | "middleRight"
...
export function alignSelectedComponents(alignment: Alignment): void {
    ...
    selectedComps.forEach((comp) => {
        ...
        if (targetX !== undefined) {
            if (alignment === "left" || alignment === "middleLeft") newX = targetX
            else if (alignment === "center") newX = targetX - comp.width / 2
            else if (alignment === "right" || alignment === "middleRight") newX = targetX - comp.width
        }
        if (targetY !== undefined) {
            if (alignment === "top") newY = targetY
            else if (alignment === "middle" || alignment === "middleLeft" || alignment === "middleRight")
                newY = targetY - comp.height / 2
